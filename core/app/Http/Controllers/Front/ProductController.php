...
class ProductController extends Controller
{
    ...
    public function productDetails($slug)
    {
        ...
        $data['product'] = Product::with('prices')->where('slug', $slug)->where('language_id', $currentLang->id)->first();
        $data['categories'] = Pcategory::where('status', 1)->where('language_id', $currentLang->id)->get();
    ...
    public function addToCart(Request $request)
    {
        ...
        $product_price = ProductPrices::whereId($price_id)->first()->price;

        $product = Product::with('prices')->where('id', $id)->first();
        ...
        Session::put('link', url()->current());
        $data['product'] = Product::with('prices')->where('slug', $product->slug)->where('language_id', $currentLang->id)->first();
        $data['categories'] = Pcategory::where('status', 1)->where('language_id', $currentLang->id)->get();
        ...
        if (strpos($price_id, ',,,') == true) {
            info('strpos');
            dd();
            $data = explode(',,,', $price_id);
            $price_id = $data[0];
            $qty = $data[1];
            ...
            if ($product->type != 'digital') {
                if (!empty($cart) && array_key_exists($price_id, $cart)) {
                    if ($product->stock < $cart[$price_id]['qty'] + $qty) {
            ...
            if (isset($cart[$price_id])) {
                $cart[$price_id]['qty'] +=  $qty;
                Session::put('cart', $cart);
                return view('front.product.details', $data);
            ...
            if ($product->type != 'digital') {
                if (!empty($cart) && array_key_exists($price_id, $cart)) {
                    if ($product->stock < $cart[$price_id]['qty'] + 1) {
                        return response()->json(['error' => 'Out of Stock']);
                    }
                } else {
                    if ($product->stock < 1) {
