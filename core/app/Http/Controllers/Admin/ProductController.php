...
class ProductController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // packages
        $packages = [];
        $list_package = $request->input('list_package');
        $data = [];
        $accounter = count($list_package);

        for ($i = 0; $i < $accounter; $i++)

            if ($i % 2 == 0) {
                $data['price'] = $list_package[$i];
                $data['desc'] = $list_package[$i + 1];
    ...
    public function edit(Request $request, $id)
    {
        ...
        $data = Product::with('prices')->findOrFail($id);
    ...
    {
        info($request);
        $ProductPrices =   ProductPrices::whereId($request->price_id)->first();
